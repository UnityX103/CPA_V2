...
- Updater endpoints are ordered CNB first, GitHub second: `https://cnb.cool/nanzhaigame-xpy/CPA_V2/-/releases/latest/download/latest.json`, then `https://github.com/UnityX103/CPA_V2/releases/latest/download/latest.json`. The native `app_update` command checks both manifests, prefers CNB at equal versions, and selects GitHub when it has a newer update; do not rely on Tauri endpoint-order fallback for freshness. Already published 0.1.32 clients still use the previous endpoint-order behavior.
