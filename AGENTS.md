...
- Version discovery queries GitHub Latest first (`https://github.com/UnityX103/CPA_V2/releases/latest/download/latest.json`), then CNB Latest only on query failure (`https://cnb.cool/nanzhaigame-xpy/CPA_V2/-/releases/latest/download/latest.json`). The native `app_update` commands download the selected version's package from its exact CNB tag first, then GitHub on download/signature failure. Never use CNB Latest to download a GitHub-selected version. Install once only after verified download. Already published 0.1.32 clients still use the old CNB-first endpoint behavior.
- GitHub Actions is the authoritative CI/release publisher. CNB asynchronously mirrors the public GitHub Latest release every 30 minutes, using anonymous GitHub reads and its own temporary CNB token. GitHub publication never waits for CNB. See `docs/deployment/cloud-release.md`; this supersedes historical CNB-first publication gates. CNB imports release tags without overwriting its pipeline branch.
...
- GitHub prepares and signs provider-specific extension index variants; CNB downloads and verifies them without a signing key. Only unsigned updater metadata may be rewritten on CNB. Binary signatures remain reusable. Mirror all Latest attachments and old-tag packages referenced by the current signed indexes.
- Use `app/scripts/mirror-github-release.mjs` for autonomous mirroring and `sync-cnb-release.mjs` for CNB OpenAPI uploads. Verify every remote size/hash, upload packages before indexes and `latest.json` last, then mark CNB Latest. Mirror failures leave GitHub published and retry later.
- GitHub credentials are not stored in this repo. Cloud publishing uses GITHUB_TOKEN and explicitly provisioned updater Secrets. Local keys stay under the user-level release configuration; `cpa-v2-release/` is staging only. Never send CNB credentials to GitHub.
